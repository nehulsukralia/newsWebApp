...
func (m PostsModel) Get(id int) (*Post, error) {
	...
	q := strings.Replace(queryTemplate, "#where#", "WHERE p.id = $1", 1) // $1 is the first arguement to the containing function, in this case is the 'id', similarly $2 is the second arguement and so on
	q = strings.Replace(q, "#orderby#", "", 1)                           // we are replacing orderby with empty string since we are only getting a single post here
	q = strings.Replace(q, "#limit#", "", 1)                             // same as above comment
